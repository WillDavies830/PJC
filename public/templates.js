const Templates = {
    ...
    resultsTable: function(results, formatTimeFn) {
      ...
      let html = `<table><thead><tr><th>Position</th><th>Runner</th><th>Race Time</th><th>Finish Time</th></tr></thead><tbody>`;
      
      results.forEach((result, index) => {
        html += `<tr><td>${index + 1}</td><td>${result.runnerNumber}</td><td>${formatTimeFn(result.raceTime)}</td><td>${new Date(result.finishTime).toLocaleTimeString()}</td></tr>`;
      });
